...
def getJhmsFromEpoch(epoch: int, currentOffset: bool = False, tz: TZ = None) -> Tuple[int, int, int, int]:
...
def getEpochFromJhms(jd: int, hour: int, minute: int, second: int, tz: TZ = None) -> int:
...
def timeEncode(tm: Union[Tuple[int, int, int], Tuple[int, int]], checkSec: bool = False) -> str:
	if len(tm) == 2:
		tm = tm + (0,)
	if checkSec:
...
def simpleTimeEncode(tm: Union[Tuple[int, int, int], Tuple[int, int], Tuple[int]]) -> str:
	if len(tm) == 1:
		return "%d" % tm
	elif len(tm) == 2:
		if tm[1] == 0:
